...
//        }
    ...
    func readAttribute(isPaging:Bool = false) ->[String:NSObject] {
        ...
        if isPaging {
            ...
            // 只需要传回跟分页有关的属性即可
            return [NSAttributedStringKey.font.rawValue:readFont(),NSAttributedStringKey.paragraphStyle.rawValue:paragraphStyle]
            
        }else{
            
            return [NSAttributedStringKey.foregroundColor.rawValue:textColor,NSAttributedStringKey.font.rawValue:readFont(),NSAttributedStringKey.paragraphStyle.rawValue:paragraphStyle]
        }
    }
